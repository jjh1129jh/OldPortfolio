			...
			if(answerArray[_pn-1][_n-1] == textArray[_pn-1][_n-1][1] || answerArray[_pn-1][_n-1] == textArray[_pn-1][_n-1][2]){
				if(top.bolPorted){
					if(top.bolPorted){//포팅이 되었을 때
						//정답일 경우
						top.opener.playerChk.contsState("<보기>와 같이 연습하세요.",textArray[_pn-1][_n-1][1],"쓰기",$(".keyboardText"+ _pn + "_" + _n).val(),100,(chanceCount[_n-1] + 1))
				...
				if(chanceCount[_n-1] == 0){
					...
					if(top.bolPorted){//포팅이 되었을 때
						top.opener.playerChk.contsState("<보기>와 같이 연습하세요.",textArray[_pn-1][_n-1][1],"쓰기",$(".keyboardText"+ _pn + "_" + _n).val(),0,(chanceCount[_n-1]))
					}
					...
					if(top.bolPorted){//포팅이 되었을 때
						top.opener.playerChk.contsState("<보기>와 같이 연습하세요.",textArray[_pn-1][_n-1][1],"쓰기",$(".keyboardText"+ _pn + "_" + _n).val(),0,(chanceCount[_n-1] + 1))
					}
	...
	for(var i=1; i<=textArray.length; i++){
		$("#contents_box").append(
			"<div id='contents"+ itostr(i) + "' class='sceneLayer left"+ i +"'>"+
				"<span class='middleTitle1'><보기>와 같이 연습하세요.</span>"+

				"<div class='develop'>"+
					"<img class='view_box' src='./common/css/img/contents/viewbox.png' alt='보기 박스'/>"+
